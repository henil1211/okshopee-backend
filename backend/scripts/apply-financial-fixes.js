...
function getBackendBaseUrl() {
  const configured = String(process.env.BACKEND_BASE_URL || process.env.PUBLIC_BACKEND_URL || '').trim();
  if (configured) {
...
async function applyFixes() {
  ...
  try {
    ...
    try {
      const refreshUrl = `${getBackendBaseUrl()}/api/state?force=1`;
      const response = await fetch(refreshUrl, { method: 'GET' });
      if (!response.ok) {
        console.warn(`⚠️ Backend cache refresh returned HTTP ${response.status}. Restart the backend if the live site still shows stale data.`);
      } else {
        console.log('✅ Backend state cache refreshed after database update.');
      }
    } catch (refreshError) {
      console.warn(`⚠️ Could not refresh backend cache automatically: ${refreshError instanceof Error ? refreshError.message : String(refreshError)}`);
      console.warn('   Restart the backend process if the live site still shows stale wallets or transactions.');
    }
